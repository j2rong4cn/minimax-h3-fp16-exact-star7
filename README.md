...
仓库提供适用于不同 NVIDIA 架构的通用示例工作流：[中文版](examples/workflows/MiniMax-H3-FP16-Chunk-Star7.json) · [English](examples/workflows/MiniMax-H3-FP16-Chunk-Star7-English.json)。新版示例包含多合一条件载入、分块、实时预览和默认关闭的一键人脸修复。载入节点会在 SM75 使用受保护 FP16，并在 SM80+ 自动采用安全 BF16，因此无需再按 RTX 20 系拆分案例。示例工作流还使用以下项目：
